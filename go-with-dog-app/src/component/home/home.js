...
function Home() {
    ...
    return <Container maxWidth="xl" id='home'>
     <Typography variant="h1" sx={{textAlign: "left" , marginTop: "15px" , fontSize: "25px"}} gutterBottom>Bienvenue sur go with dog! Le site qui réferencie les adresses dogfriendly</Typography>
        <Typography variant="h2" sx={{textAlign: "center" , marginTop: "35px"}} gutterBottom>Dernières balades ajoutées</Typography>
        {loading ? (
            ...
            <Box sx={{ maxWidth: '100%' , marginTop: "2%"}}>
                ...
                return (
                <Card sx={{ width: '300px' , display: 'inline-block'  , marginRight: "25px"}} key={id + ballade_name} color="success">
                    <CardHeader
                        title={ ballade_name }
                    />
                    <CardMedia
                        component="img"
                        height="160"
                        image={`https://api.gowithdog.fr/storage/uploads/ballades/${ballade_image}`}
                        alt={ballade_name}
                    />
                    <CardContent>
